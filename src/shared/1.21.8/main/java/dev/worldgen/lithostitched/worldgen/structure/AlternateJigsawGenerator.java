...
public class AlternateJigsawGenerator {
    public static Optional<Structure.GenerationStub> generate(Structure.GenerationContext context, AlternateJigsawConfig config, boolean vanilla, int size, BlockPos pos, PoolAliasLookup aliasLookup) {
        ...
        if (startJigsawName.isPresent()) {
            ...
            if (optional.isEmpty()) {
                Lithostitched.LOGGER.error("No starting jigsaw {} found in start pool {}", startJigsawName.get(), config.startPool().unwrapKey().map(key -> key.location().toString()).orElse("<unregistered>"));
                return Optional.empty();
            }
        ...
        return Optional.of(new Structure.GenerationStub(new BlockPos(x, y, z), (collector) -> {
            ...
            if (size > 0) {
                ...
                generatePieces(context, vanilla, size, config.useExpansionHack(), chunkGenerator, structureTemplateManager, heightView, random, templatePoolRegistry, poolStructurePiece, pieces, voxelShape, aliasLookup, config.liquidSettings());
    ...
    static final class StructurePoolGenerator {
        ...
        private boolean findValidChildPiece(List<StructurePoolElement> elements, PoolElementStructurePiece parentPiece, StructureTemplate.JigsawBlockInfo anchorJigsaw, MutableObject<VoxelShape> mutableObject2, int k, int depth, boolean useExpansionHack, LevelHeightAccessor world, PoolAliasLookup aliasLookup, LiquidSettings liquidSettings) {
            StructureTemplate.StructureBlockInfo anchorInfo = anchorJigsaw.info();
