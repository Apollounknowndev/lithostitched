...
public record Region(Optional<ResourceKey<Region>> name, ResourceKey<LevelStem> dimension, int weight) {
	...
	public static Region create(ResourceKey<LevelStem> level, int weight) {
		return new Region(Optional.empty(), level, weight);
	}
	
	public static Region create(ResourceKey<Region> key, ResourceKey<Level> level, int weight) {
		return new Region(Optional.of(key), Registries.levelToLevelStem(level), weight);
	}
}
