...
public class DelegatingPoolElement extends StructurePoolElement {
    public static final Codec<DelegatingPoolElement> CODEC = DelegatingConfig.CODEC.xmap(DelegatingPoolElement::new, DelegatingPoolElement::config).codec();
    ...
    protected DelegatingPoolElement(StructurePoolElement delegate, Optional<Integer> minDepth, Optional<Integer> forcedCount, Optional<Integer> maxCount) {
        this(new DelegatingConfig(delegate, minDepth.map(min -> Optional.of(new InclusiveRange<>(min, Integer.MAX_VALUE))).orElse(Optional.empty()), forcedCount, maxCount, Optional.empty()));
    }
    ...
    public boolean place(StructureTemplateManager structureTemplateManager, WorldGenLevel worldGenLevel, StructureManager structureManager, ChunkGenerator chunkGenerator, BlockPos blockPos, BlockPos blockPos1, Rotation rotation, BoundingBox boundingBox, RandomSource randomSource, boolean b) {
        return this.config.delegate().place(structureTemplateManager, worldGenLevel, structureManager, chunkGenerator, blockPos, blockPos1, rotation, boundingBox, randomSource, b);
    }
