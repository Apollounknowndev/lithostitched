...
public record VinesConfig(SimpleWeightedRandomList<Block> blocks, Optional<HolderSet<Block>> canPlaceOn, IntProvider maxLength) implements FeatureConfiguration {
    private static final SimpleWeightedRandomList<Block> DEFAULT_BLOCK = SimpleWeightedRandomList.<Block>builder().add(Blocks.VINE, 1).build();

    public static final Codec<VinesConfig> CODEC = ExtraCodecs.validate(RecordCodecBuilder.create(instance -> instance.group(
    ...
    ).apply(instance, VinesConfig::new)), VinesConfig::validate);

    private DataResult<VinesConfig> validate() {
        if (this.blocks.unwrap().stream().map(WeightedEntry.Wrapper::getData).anyMatch(block -> !(block instanceof VineBlock))) {
